...
public abstract class BaseStrokeContent
  ...
  BaseStrokeContent(final AnideroDrawable anideroDrawable, BaseLayer layer, Paint.Cap cap,
  ...
  @Override public void draw(Canvas canvas, Matrix parentMatrix, int parentAlpha) {
    Anidero.beginSection("StrokeContent#draw");
    if (Utils.hasZeroScaleAxis(parentMatrix)) {
      Anidero.endSection("StrokeContent#draw");
      return;
    }
    ...
    for (int i = 0; i < pathGroups.size(); i++) {
      ...
      if (pathGroup.trimPath != null) {
        ...
        Anidero.beginSection("StrokeContent#buildPath");
        path.reset();
        for (int j = pathGroup.paths.size() - 1; j >= 0; j--) {
          path.addPath(pathGroup.paths.get(j).getPath(), parentMatrix);
        }
        Anidero.endSection("StrokeContent#buildPath");
        Anidero.beginSection("StrokeContent#drawPath");
        canvas.drawPath(path, paint);
        Anidero.endSection("StrokeContent#drawPath");
      }
    }
    Anidero.endSection("StrokeContent#draw");
  }
  ...
  private void applyTrimPath(Canvas canvas, PathGroup pathGroup, Matrix parentMatrix) {
    Anidero.beginSection("StrokeContent#applyTrimPath");
    if (pathGroup.trimPath == null) {
      Anidero.endSection("StrokeContent#applyTrimPath");
      return;
    }
    path.reset();
    for (int j = pathGroup.paths.size() - 1; j >= 0; j--) {
  ...
  @Override public void getBounds(RectF outBounds, Matrix parentMatrix, boolean applyParents) {
    Anidero.beginSection("StrokeContent#getBounds");
    path.reset();
    for (int i = 0; i < pathGroups.size(); i++) {
      PathGroup pathGroup = pathGroups.get(i);
    ...
    Anidero.endSection("StrokeContent#getBounds");
  }

  private void applyDashPatternIfNeeded(Matrix parentMatrix) {
    Anidero.beginSection("StrokeContent#applyDashPattern");
    if (dashPatternAnimations.isEmpty()) {
      Anidero.endSection("StrokeContent#applyDashPattern");
      return;
    }
