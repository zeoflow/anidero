package com.zeoflow.anidero.manager;
...
import com.zeoflow.anidero.ImageAssetDelegate;
...
public class ImageAssetManager {
  ...
  private final Map<String, AnideroImageAsset> imageAssets;

  public ImageAssetManager(Drawable.Callback callback, String imagesFolder,
      ImageAssetDelegate delegate, Map<String, AnideroImageAsset> imageAssets) {
    ...
    if (!(callback instanceof View)) {
      Logger.warning("AnideroDrawable must be inside of a view for images to work.");
  ...
  @Nullable public Bitmap updateBitmap(String id, @Nullable Bitmap bitmap) {
    if (bitmap == null) {
      AnideroImageAsset asset = imageAssets.get(id);
    ...
    try {
      if (TextUtils.isEmpty(imagesFolder)) {
        throw new IllegalStateException("You must set an images folder before loading an image." +
            " Set it with AnideroComposition#setImagesFolder or AnideroDrawable#setImagesFolder");
      }
