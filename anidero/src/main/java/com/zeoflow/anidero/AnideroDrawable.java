...
import com.zeoflow.anidero.manager.FontAssetManager;
import com.zeoflow.anidero.manager.ImageAssetManager;
import com.zeoflow.anidero.model.KeyPath;
import com.zeoflow.anidero.model.Marker;
import com.zeoflow.anidero.model.layer.CompositionLayer;
import com.zeoflow.anidero.parser.LayerParser;
import com.zeoflow.anidero.utils.Logger;
import com.zeoflow.anidero.utils.AnideroValueAnimator;
...
/**
 * This can be used to show an anidero animation in any place that would normally take a drawable.
 *
 * @see <a href="http://airbnb.io/anidero">Full Documentation</a>
 */
@SuppressWarnings({"WeakerAccess", "unused"})
public class AnideroDrawable extends Drawable implements Drawable.Callback, Animatable {
  private static final String TAG = AnideroDrawable.class.getSimpleName();

  private interface LazyCompositionTask {
    void run(AnideroComposition composition);
  }

  private final Matrix matrix = new Matrix();
  private AnideroComposition composition;
  private final AnideroValueAnimator animator = new AnideroValueAnimator();
   ...
   * <p>
   * <p>
   * If you use AnideroDrawable directly, you MUST call {@link #recycleBitmaps()} when you
   * are done. Calling {@link #recycleBitmaps()} doesn't have to be final and {@link AnideroDrawable}
  ...
  /**
   * Create a composition with {@link AnideroCompositionFactory}
   *
   * @return True if the composition is different from the previously set composition, false otherwise.
   */
  public boolean setComposition(AnideroComposition composition) {
   ...
   * @see AnideroAnimationView#setRenderMode(RenderMode)
   */
  ...
  public void draw(@NonNull Canvas canvas) {
    ...
    Anidero.beginSection("Drawable#draw");

    if (safeMode) {
      try {
        drawInternal(canvas);
      } catch (Throwable e) {
        Logger.error("Anidero crashed in draw!", e);
      }
    } else {
      drawInternal(canvas);
    }

    Anidero.endSection("Drawable#draw");
  }
...
// <editor-fold desc="animator">
  ...
  public void playAnimation() {
    if (compositionLayer == null) {
      lazyCompositionTasks.add(new LazyCompositionTask() {
        @Override
        public void run(AnideroComposition composition) {
          playAnimation();
        }
      });
      return;
  ...
  public boolean isAnimating() {
    // On some versions of Android, this is called from the AnideroAnimationView constructor, before animator was created.
    // https://github.com/airbnb/anidero-android/issues/1430
    if (animator == null) {
      return false;
    }
...
// </editor-fold>
  ...
  public AnideroComposition getComposition() {
    return composition;
  }
  ...
  public <T> void addValueCallback(
      final KeyPath keyPath, final T property, final AnideroValueCallback<T> callback) {
    if (compositionLayer == null) {
      lazyCompositionTasks.add(new LazyCompositionTask() {
        @Override
        public void run(AnideroComposition composition) {
    ...
    if (invalidate) {
      invalidateSelf();
      if (property == AnideroProperty.TIME_REMAP) {
   ...
   * drawable.addValueCallback(yourKeyPath, AnideroProperty.COLOR) { yourColor }
   */
  public <T> void addValueCallback(KeyPath keyPath, T property,
                                   final SimpleAnideroValueCallback<T> callback) {
    addValueCallback(keyPath, property, new AnideroValueCallback<T>() {
      @Override
      public T getValue(AnideroFrameInfo<T> frameInfo) {
  ...
  public Bitmap updateBitmap(String id, @Nullable Bitmap bitmap) {
    ...
    if (bm == null) {
      Logger.warning("Cannot update bitmap. Most likely the drawable is not added to a View " +
          "which prevents Anidero from getting a Context.");
      return null;
    }
