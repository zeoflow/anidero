...
public class CompositionLayer extends BaseLayer {
  ...
  public CompositionLayer(AnideroDrawable anideroDrawable, Layer layerModel, List<Layer> layerModels,
                          AnideroComposition composition) {
    ...
    for (int i = layerModels.size() - 1; i >= 0; i--) {
      ...
      BaseLayer layer = BaseLayer.forModel(lm, anideroDrawable, composition);
      if (layer == null) {
        continue;
      }
    ...
    for (int i = 0; i < layerMap.size(); i++) {
      ...
      // https://github.com/airbnb/anidero-android/issues/524
      if (layerView == null) {
        continue;
      }
  ...
  @Override void drawLayer(Canvas canvas, Matrix parentMatrix, int parentAlpha) {
    Anidero.beginSection("CompositionLayer#draw");
    newClipRect.set(0, 0, layerModel.getPreCompWidth(), layerModel.getPreCompHeight());
    parentMatrix.mapRect(newClipRect);

    // Apply off-screen rendering only when needed in order to improve rendering performance.
    boolean isDrawingWithOffScreen = anideroDrawable.isApplyingOpacityToLayersEnabled() && layers.size() > 1 && parentAlpha != 255;
    ...
    canvas.restore();
    Anidero.endSection("CompositionLayer#draw");
  }
  ...
  @Override public void setProgress(@FloatRange(from = 0f, to = 1f) float progress) {
    ...
    if (timeRemapping != null) {
      ...
      // https://github.com/airbnb/anidero-android/pull/766
      // Ignore this offset for calculating time-remapping because time-remapping value is based on original duration.
      float durationFrames = anideroDrawable.getComposition().getDurationFrames() + 0.01f;
  ...
  public <T> void addValueCallback(T property, @Nullable AnideroValueCallback<T> callback) {
    ...
    if (property == AnideroProperty.TIME_REMAP) {
      if (callback == null) {
        ...
        timeRemapping = new ValueCallbackKeyframeAnimation<>((AnideroValueCallback<Float>) callback);
