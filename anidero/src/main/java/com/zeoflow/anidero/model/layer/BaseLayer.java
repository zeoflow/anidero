...
import com.zeoflow.anidero.animation.content.DrawingContent;
...
import com.zeoflow.anidero.animation.keyframe.MaskKeyframeAnimation;
import com.zeoflow.anidero.animation.keyframe.TransformKeyframeAnimation;
import com.zeoflow.anidero.model.KeyPath;
import com.zeoflow.anidero.model.KeyPathElement;
import com.zeoflow.anidero.model.content.Mask;
...
public abstract class BaseLayer
  ...
  static BaseLayer forModel(
      Layer layerModel, AnideroDrawable drawable, AnideroComposition composition) {
  ...
  final AnideroDrawable anideroDrawable;
  ...
  BaseLayer(AnideroDrawable anideroDrawable, Layer layerModel) {
  ...
  public void draw(Canvas canvas, Matrix parentMatrix, int parentAlpha) {
    Anidero.beginSection(drawTraceName);
    if (!visible || layerModel.isHidden()) {
      Anidero.endSection(drawTraceName);
      return;
    }
    buildParentLayerListIfNeeded();
    Anidero.beginSection("Layer#parentMatrix");
    matrix.reset();
    matrix.set(parentMatrix);
    for (int i = parentLayers.size() - 1; i >= 0; i--) {
      matrix.preConcat(parentLayers.get(i).transform.getMatrix());
    }
    Anidero.endSection("Layer#parentMatrix");
    int opacity = transform.getOpacity() == null ? 100 : transform.getOpacity().getValue();
    int alpha = (int)
        ((parentAlpha / 255f * (float) opacity / 100f) * 255);
    if (!hasMatteOnThisLayer() && !hasMasksOnThisLayer()) {
      matrix.preConcat(transform.getMatrix());
      Anidero.beginSection("Layer#drawLayer");
      drawLayer(canvas, matrix, alpha);
      Anidero.endSection("Layer#drawLayer");
      recordRenderTime(Anidero.endSection(drawTraceName));
      return;
    }

    Anidero.beginSection("Layer#computeBounds");
    ...
    Anidero.endSection("Layer#computeBounds");

    if (!rect.isEmpty()) {
      Anidero.beginSection("Layer#saveLayer");
      contentPaint.setAlpha(255);
      Utils.saveLayerCompat(canvas, rect, contentPaint);
      Anidero.endSection("Layer#saveLayer");
      ...
      if (hasMatteOnThisLayer()) {
        Anidero.beginSection("Layer#drawMatte");
        Anidero.beginSection("Layer#saveLayer");
        ...
        Anidero.beginSection("Layer#restoreLayer");
        canvas.restore();
        Anidero.endSection("Layer#restoreLayer");
        Anidero.endSection("Layer#drawMatte");
      }
  ...
  private void recordRenderTime(float ms) {
    anideroDrawable.getComposition()
        .getPerformanceTracker().recordRenderTime(layerModel.getName(), ms);

  }

  private void clearCanvas(Canvas canvas) {
    Anidero.beginSection("Layer#clearLayer");
    ...
    Anidero.endSection("Layer#clearLayer");
  }
